...
def cadastro_medico(request):
    ...
    if request.method == 'GET':
        # context -> é um dicionario do python que vai ser acessivel no html
        ...
        #para pegar arquivos usar FILES ao inves de POST
        ...
        dados_medicos = DadosMedicos(
            ...
            #esse campo abaixo espera uma INSTANCIA de ESPECIALIDADE | Para fazer funcionar peça que crie o campo especialidade onde o id seja = ao id que foi recebido pelo html
...
@login_required
def abir_horario(request):

    #não deixa acessar a página
    ...
    if request.method == 'GET':
        ...
        return render(request, 'abir_horario.html', {'dados_medicos': dados_medicos, 'datas_abertas':datas_abertas, 'is_medico':is_medico(request.user)})
...
def finalizar_consulta(request, id_consulta):
    ...
    #TODO identificar pontos do código que pode colocar validação de segurança

    #TODO fazer a mesma coisa para ao inves de finalizar, cancelar
...
def dashboard(request):
    ...
    consultas = Consulta.objects.filter(data_aberta__user=request.user).filter(data_aberta__data__range=[datetime.now().date() - timedelta(days=7), datetime.now().date() + timedelta(days=1)]).annotate(data_truncada=TruncDate('data_aberta__data')).values('data_aberta__data').annotate(quantidade=Count('id'))
    # novos filtros: __range -> pega tudo em um intervalo assim: [começo_data, fim_data]
    # .values() -> pega apenas os valores! e annotate() faz a contagem e o Count é do django | from django.db.models import Count
